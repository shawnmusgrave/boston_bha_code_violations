...
TKTK -- GitHub link to API call code

TKTK -- GitHub link to Cutting down to serious complaints . 
...
Once the list of housing development addresses had been expanded to include all relevant street addresses (and combinations thereof) for which code violations and complaints might be listed, the addresses needed to be formatted for comparison to the violation/complaint database, which uses the following format for addresses: 
	
	104 First Ave Charlestown MA 02129

That is, the database uses two spaces ("  ") rather than commas to separate address components (street address, neighborhood, state and zipcode). All street types needed to be reformatted (for example, "Street" changed to "St" and "Avenue" to "Ave"), and some common street name abbreviations expanded (for example, "Mass Ave" expanded to "Massachusetts Ave"). 

All cleaning was done by hand (and using "find and replace") in Google Drive, rather than via regular expressions, given the relatively small number of entries and wide range of modifications required. The full listing of BHA development addresses can be found TKTK. 
